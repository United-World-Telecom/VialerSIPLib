...
- (BOOL)startEndpointWithEndpointConfiguration:(VSLEndpointConfiguration  * _Nonnull)endpointConfiguration error:(NSError **)error {
    ...
    NSLog(@"Creating new PJSIP Endpoint instance.");
...
- (void)destroyPJSUAInstance {
    NSLog(@"PJSUA was already running destroying old instance.");
    ...
    if (status != PJ_SUCCESS) {
        char statusmsg[PJ_ERR_MSG_SIZE];
        pj_strerror(status, statusmsg, sizeof(statusmsg));
        NSLog(@"Error stopping SIP Endpoint, status: %s", statusmsg);
    }
...
static void onCallState(pjsua_call_id callId, pjsip_event *event) {
    ...
    NSLog(@"PJSUA callback: call state changed to %@.", VSLCallStateString(callInfo.state));
    ...
    if (account) {
        ...
        if (call) {
            ...
            NSLog(@"Received updated CallState(%@) for UNKNOWN call(id: %d)", VSLCallStateString(callInfo.state) , callId);
        }
    }
}
 ...
 */
static void onCallMediaState(pjsua_call_id call_id) {
    NSLog(@"PJSUA callback: media state in the call has changed.");
    pjsua_call_info callInfo;
    ...
    if (account) {
        ...
        NSLog(@"Received MediaState update for call:%@", call.uuid.UUIDString);
        if (call) {
 ...
 */
static void onRegStarted2(pjsua_acc_id acc_id, pjsua_reg_info *info) {
    NSLog(@"PJSUA callback: registration or unregistration has been initiated.");
}
 ...
 */
static void onRegState2(pjsua_acc_id acc_id, pjsua_reg_info *info) {
    NSLog(@"PJSUA callback: registration status has changed.");
 ...
 */
static void onCallMediaEvent(pjsua_call_id call_id, unsigned med_idx, pjmedia_event *event) {
    NSLog(@"PJSUA callback: media event.");
    
    #if PJSUA_HAS_VIDEO
        if (event->type == PJMEDIA_EVENT_FMT_CHANGED) {
            char event_name[5];
            NSLog(@"Media event %s", pjmedia_fourcc_name(event->type, event_name));
            pjsua_call_info ci;
 ...
 */
static void onTxStateChange(pjsua_call_id call_id, pjsip_transaction *tx, pjsip_event *event) {
    NSLog(@"PJSUA callback: transaction within the call has changed state.");
    pjsua_call_info callInfo;
    pjsua_call_get_info(call_id, &callInfo);
 ...
 */
static void onIncomingCall(pjsua_acc_id acc_id, pjsua_call_id call_id, pjsip_rx_data *rdata) {
    NSLog(@"PJSUA callback: incoming call.");
    ...
    if (account) {
        ...
        if (call) {
            ...
            NSLog(@"Could not find a call with if %d.", call_id);
        }
        call = nil;
    } else {
        NSLog(@"Could not accept incoming call. No account found with ID:%d", acc_id);
    }
}
...
static void onCallTransferStatus(pjsua_call_id callId, int statusCode, const pj_str_t *statusText, pj_bool_t final, pj_bool_t *continueNotifications) {
    NSLog(@"PJSUA callback: the status of previously sent call transfer request.");
    VSLCall *call = [[VSLEndpoint sharedEndpoint].callManager callWithCallId:callId];
    if (call) {
    ...
    switch (callState) {
        ...
        case VSLCallStateConfirmed: {
            if (!self.monitoringCalls) {
                for (VSLAccount *account in self.accounts) {
                    if ([self.callManager firstCallForAccount:account]) {
                        NSLog(@"Starting network monitor");
...
- (void)stopNetworkMonitoring {
    ...
    if (!activeCallForAnyAccount) {
        NSLog(@"No active calls for any account, stopping network monitor");
        @try {
            [[NSNotificationCenter defaultCenter] removeObserver:self name:VSLNetworkMonitorChangedNotification object:nil];
        } @catch (NSException *exception) {
            NSLog(@"Exception on removing the reachability change notification.");
        }
...
static void onIpChangeProgress(pjsua_ip_change_op op, pj_status_t status, const pjsua_ip_change_op_info *info) {
    ...
    switch (op) {
        case PJSUA_IP_CHANGE_OP_NULL: {
            NSLog(@"Hasn't start ip change process, status: %s", statusmsg);
            break;
        }
        case PJSUA_IP_CHANGE_OP_RESTART_LIS: {
            NSLog(@"The restart listener process, status: %s", statusmsg);
            break;
        }
        case PJSUA_IP_CHANGE_OP_ACC_SHUTDOWN_TP: {
            NSLog(@"The shutdown transport process, statust: %s", statusmsg);
            break;
        }
        case PJSUA_IP_CHANGE_OP_ACC_UPDATE_CONTACT: {
            NSLog(@"The update contact process, status: %s", statusmsg);
            break;
        }
        case PJSUA_IP_CHANGE_OP_ACC_HANGUP_CALLS: {
            NSLog(@"The hanging up call process, status: %s", statusmsg);
            break;
        }
        case PJSUA_IP_CHANGE_OP_ACC_REINVITE_CALLS: {
            NSLog(@"The re-INVITE call process, status: %s", statusmsg);
            break;
        }
        default:
    ...
    if (![packet isEqualToString: @""]) {
        ...
        if ([VSLEndpoint sharedEndpoint].missedCallBlock && reason != VSLCallTerminateReasonUnknown) {
            ...
            if (call) {
                ...
                NSLog(@"Call was terminated for reason: %@", VSLCallTerminateReasonString(reason));
                [VSLEndpoint sharedEndpoint].missedCallBlock(call);
            } else {
                NSLog(@"Received updated CallState(%@) for UNKNOWN call(id: %d)", VSLCallStateString(callInfo.state) , call_id);
            }
        }
    }
}
...
static void onTransportStateChanged(pjsip_transport *tp, pjsip_transport_state state, const pjsip_transport_state_info *info) {
    NSLog(@"Transport state changed to: %@", VSLTransportStateName(state));
...
static void onNatDetect(const pj_stun_nat_detect_result *res){
    if (res->status != PJ_SUCCESS) {
        NSLog(@"NAT detection failed %@", res->status ? @"YES" : @"NO");
    } else {
        NSLog(@"NAT detected as %s", res->nat_type_name);
    }
}
