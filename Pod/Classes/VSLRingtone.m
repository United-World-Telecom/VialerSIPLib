...
- (void)configureAudioSessionBeforeRingtoneIsPlayed {
    NSLog(@"Configuring Audio before playing ringtone");
    ...
    if (!setCategorySuccess) {
        if (setCategoryError != NULL) {
            NSLog(@"Error setting audioplayer category: %@", setCategoryError);
        }
    }
    ...
    if (!overrideOutputAudioPortSuccess) {
        if (overrideOutputAudioPortError != NULL) {
            NSLog(@"Error overriding audio port: %@", overrideOutputAudioPortError);
        }
    }
    ...
    if (!setActiveSuccess) {
        if (setActiveError != NULL) {
            NSLog(@"Error activatiing audio: %@", setActiveError);
        }
    }
}

- (void)configureAudioSessionAfterRingtoneStopped {
    NSLog(@"Configuring Audio after ringtone has stoped");
    AVAudioSession *audioSession = [AVAudioSession sharedInstance];
